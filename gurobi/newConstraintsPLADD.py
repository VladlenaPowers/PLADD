...
try:
    ...
    length =  [[2,8,2,2,8,2],[4,6,4,4,6,4],[6,6,6,4]]
    T = 20
    K = 5
    ...
    #     jobStartTimes = []
    #     for j in range(n):
    #         jobStartTimes.append(pladdLPM.addVar(lb=0.0, vtype=GRB.CONTINUOUS, name='W_m{0}_j{1}'.format(m, j)))
    #     w.append(jobStartTimes)
    ...
    for m in range(M):
        n = len(length[m])
        for j in range(n):
            pladdLPM.addConstr(quicksum(x[m][j]) == 1.0, 'C: ever y job scheduled for server {0}'.format(m))
    ...
    for m in range(M):
        ...
        for i in range(n-1):
            ...
            exprR = LinExpr(0.0)
            for t in range(T+sumOfLengthServerJobs[m]+1):
                exprL.addTerms(t, x[m][i][t])
                exprR.addTerms(t, x[m][i+1][t])
            pladdLPM.addConstr(exprL + length[m][i] <=  exprR)
    ...
    #         exprW = LinExpr(0.0)
    #         for t in range(T):
    #             exprW.addTerms(t,x[m][j][t])
    #         pladdLPM.addConstr(w[m][j] ==  exprW, 'C: jobs before job{0} must end before job{1} starts for server{2}'.format(j, j, m))
    ...
    #     n = len(length[m])
    #     for i in range(n-1):
    #         pladdLPM.addConstr(w[m][i] + length[m][i] <= w[m][i+1])
